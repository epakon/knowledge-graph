...
- **`adapters/engine/markdown/addons/openwiki.md`** — OpenWiki addon: upstream codebase documentation generator, references [OpenWiki](https://github.com/langchain-ai/openwiki). Covers: prompting OpenWiki with `spec/page-templates.md` and `spec/schema.yaml` to produce KG-format draft nodes directly; what fields the LLM fills from source code vs. what stays as `<!-- TODO -->` for domain expert review; pipeline diagram; node type mapping; CI integration pattern; what always requires manual authoring (business definitions, reified edges, verified queries).
- **`examples/openwiki-kg-example.md`** — Generic example of an OpenWiki-generated KG draft Table node (based on a dbt reporting view with dual-row clearing pattern). Demonstrates filled fields (physical columns, semantic annotations, joins, caveats) alongside `<!-- TODO -->` placeholders for domain-expert-only fields.

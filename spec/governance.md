...
| **Agent functional overlap** | Two `Agent` pages built independently converge to near-identical `uses` sets over time, with neither author aware of the other | New check (`agent_overlap_review` in `schema.yaml`): Jaccard similarity of `uses` target sets across `Active` agents, reported for review rather than auto-flagged as a duplicate — see `consumption-layer.md` §8 for the reasonable-overlap test and merge procedure. Unlike the other rows in this table, this one is informational by design, not a pass/fail check: the goal is a review prompt, not a verdict. |
| **Spec doc drift from `schema.yaml`** | A node type or edge kind's properties/valid sources/targets were changed in `schema.yaml` but a narrative doc (`logical-layer.md`, `conceptual-layer.md`, `consumption-layer.md`, `link-format.md`, an engine or connector adapter doc) still shows the old definition, since most of them restate a subset of `schema.yaml` for readability rather than linking to it | New check: for each kind/type name in `schema.yaml`, grep `spec/` and `adapters/` for the same name and flag any doc whose stated source/target/property list doesn't match `schema.yaml`'s. This is a spec-document check, not a graph-data check — it runs against the repo, not the node/edge index. |
